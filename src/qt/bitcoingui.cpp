...
// Copyright (c) 2019 The Artax developers
                                                                            ...
                                                                            spinnerFrame(0)
{
    ...
    QString windowTitle = tr("Artax Core") + " - ";
#ifdef ENABLE_WALLET
...
void BitcoinGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and artax: URIs)"));
    ...
    QSettings settings;
    if (settings.value("fShowMerchantnodesTab").toBool()) {
        merchantnodeAction = new QAction(QIcon(":/icons/merchantnodes"), tr("&Merchantnodes"), this);
        merchantnodeAction->setStatusTip(tr("Browse merchantnodes"));
        merchantnodeAction->setToolTip(merchantnodeAction->statusTip());
        merchantnodeAction->setCheckable(true);
#ifdef Q_OS_MAC
        merchantnodeAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_6));
#else
        merchantnodeAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
#endif
        tabGroup->addAction(merchantnodeAction);
        connect(merchantnodeAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
        connect(merchantnodeAction, SIGNAL(triggered()), this, SLOT(gotoMerchantnodePage()));
    }
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About Artax Core"), this);
    aboutAction->setStatusTip(tr("Show information about Artax Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Artax"));
    ...
    openMNConfEditorAction = new QAction(QIcon(":/icons/edit"), tr("Open &Merchantnode Configuration File"), this);
    openMNConfEditorAction->setStatusTip(tr("Open Merchantnode configuration file"));
    ...
    openAction->setStatusTip(tr("Open a artax: URI or payment request"));
    openBlockExplorerAction = new QAction(QIcon(":/icons/explorer"), tr("&Blockchain explorer"), this);
    openBlockExplorerAction->setStatusTip(tr("Block explorer window"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Artax Core help message to get a list with possible Artax command-line options"));
...
void BitcoinGUI::createToolBars()
{
    if (walletFrame) {
        ...
        QSettings settings;
        if (settings.value("fShowMerchantnodesTab").toBool()) {
            toolbar->addAction(merchantnodeAction);
        }
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    QSettings settings;
    if (settings.value("fShowMerchantnodesTab").toBool()) {
        merchantnodeAction->setEnabled(enabled);
    }
...
void BitcoinGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Artax Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::gotoMerchantnodePage()
{
    QSettings settings;
    if (settings.value("fShowMerchantnodesTab").toBool()) {
        merchantnodeAction->setChecked(true);
        if (walletFrame) walletFrame->gotoMerchantnodePage();
    }
}
...
void BitcoinGUI::setNumBlocks(int count)
{
    ...
    if (merchantnodeSync.IsBlockchainSynced()) {
        ...
        strSyncStatus = QString(merchantnodeSync.GetSyncStatus().c_str());
...
{
    QString strTitle = tr("Artax Core"); // default title
